...
def rebuild_nemo(expname, leg, REBUILD_DIR, TMP_DIR):
    ...
    for kind in ['restart', 'restart_ice']: 
        ...
        filelist = glob.glob(os.path.join(EXP_DIR, 'restart', leg.zfill(3), expname + '*_' + kind + '_????.nc'))
        timestep = os.path.basename(filelist[0]).split('_')[1]
...
if __name__ == "__main__":
    ...
    TMP_DIR = os.path.join("/lus/h2resw01/scratch/ccpd/martini", expname, leg.zfill(3))
    REBUILD_DIR="/ec/res4/hpcperm/itas/src/rebuild_nemo"
    os.makedirs(TMP_DIR, exist_ok=True)

    if args.rebuild:
        rebuild_nemo(expname=expname, leg=leg, REBUILD_DIR=REBUILD_DIR, TMP_DIR=TMP_DIR)
