...
def parse_args():
    ...
    parser.add_argument("--iave", action="store_true", help="Interp Average")
    parser.add_argument("--mave", action="store_true", help="Moving Average")
    parser.add_argument("--orca1", action="store_true", help="ORCA1")
    parser.add_argument("--orca2", action="store_true", help="ORCA2")
...
if __name__ == "__main__":
    ...
    if args.nave:
        ...
        for field in ['to', 'so']:
            ...
            if args.orca2:
                ave[f'{field}m'] = data[f'{field}'].isel(z=slice(0,9)).weighted(vol.isel(z=slice(0,9))).mean(dim=['z', 'y', 'x']).values.flatten()
                fieldnames.append(f'{field}m')
                ave[f'{field}p'] = data[f'{field}'].isel(z=slice(10,20)).weighted(vol.isel(z=slice(10,20))).mean(dim=['z', 'y', 'x']).values.flatten()
                fieldnames.append(f'{field}p')
                ave[f'{field}b'] = data[f'{field}'].isel(z=slice(21,30)).weighted(vol.isel(z=slice(21,30))).mean(dim=['z', 'y', 'x']).values.flatten()
                fieldnames.append(f'{field}b')

        # 2d fields
        for field in ['tos', 'sos', 'heatc', 'saltc', 'qsr_oce', 'qns_oce', 'qt_oce']:
            ave[f'{field}g'] = data[f'{field}'].weighted(area).mean(dim=['y', 'x']).values.flatten()            
            fieldnames.append(f'{field}g')
    ...
    if args.iave:
        ...
        # 3d fields
        for field in ['to', 'so']:
            aux[f'{field}g'] = data[f'{field}'].weighted(vol).mean(dim=['z', 'y', 'x'])
            fieldnames.append(f'{field}g')
            if args.orca1:
                ...
                fieldnames.append(f'{field}b')
            if args.orca2:
                aux[f'{field}m'] = data[f'{field}'].isel(z=slice(0,9)).weighted(vol.isel(z=slice(0,9))).mean(dim=['z', 'y', 'x']).values.flatten()
                fieldnames.append(f'{field}m')
                aux[f'{field}p'] = data[f'{field}'].isel(z=slice(10,20)).weighted(vol.isel(z=slice(10,20))).mean(dim=['z', 'y', 'x']).values.flatten()
                fieldnames.append(f'{field}p')
                aux[f'{field}b'] = data[f'{field}'].isel(z=slice(21,30)).weighted(vol.isel(z=slice(21,30))).mean(dim=['z', 'y', 'x']).values.flatten()
                fieldnames.append(f'{field}b')

        # 2d fields
        for field in ['tos', 'sos', 'heatc', 'saltc', 'qsr_oce', 'qns_oce', 'qt_oce']:
            aux[f'{field}g'] = data[f'{field}'].weighted(area).mean(dim=['y', 'x'])
            fieldnames.append(f'{field}g')

        # compute moving average to remove the seasonal component from all fields (expect time)
        for field in fieldnames[1:]:
            ave[f'{field}'] = gt.interp_average(aux['time'], aux[f'{field}'], 12)

        # write output
        output = os.path.join('time_series_' + startyear + '-' + endyear + '_iave.dat')
        with open( os.path.join(dirs['tmp'], output), 'w') as file:
            for i in range(6,timef-6):
                row = [f"{ave[field][i]:<5}" for field in fieldnames]
                print(" ".join(row), file=file)
