...
def drawing(figname):
    ...
    timeseries(expname='lfr0', startyear=1990, endyear=2399, varlabel='so', reader='post', metric='reldiff', color='gray', linestyle='-', label='REF')

    # comparison with EOF experiments
    timeseries(expname='FE01', startyear=1990, endyear=2139, varlabel='so', reader='post', metric='reldiff', color='red', linestyle='-', label='EOF-T')
    timeseries(expname='FE02', startyear=1990, endyear=2089, varlabel='so', reader='post', metric='reldiff', color='blue', linestyle='-', label='EOF-TS')
    ...
    plt.title('Timeseries of reldiff metric of global mean salinity \n wrt REF [2390-2399] ')
