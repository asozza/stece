...
subprocess.call(["cdo", "-f", "grb2", "gp2spl", f"{OUTDIR}/grid_point_new.nc", f"{OUTDIR}/{OIFS_SPECTRAL}"])

OIFS_INIT = "ICMGGECE4INIT"
subprocess.call(["cdo", "--eccodes", "-f", "nc4", "copy", f"{INDIR}/{OIFS_INIT}", f"{OUTDIR}/init.nc"])
init = xr.open_dataset(f"{OUTDIR}/init.nc")
print(init)
init['al'] = init['al'] + 0.05
init.to_netcdf(f"{OUTDIR}/init_new.nc")

subprocess.call(["cdo", "--eccodes", "-f", "grb", f"-setgrid,{INDIR}/{OIFS_INIT}", f"{OUTDIR}/init_new.nc", f"{OUTDIR}/{OIFS_INIT}"])
